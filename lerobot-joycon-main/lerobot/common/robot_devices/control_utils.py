...
def control_loop(
    ...
    while timestamp < control_time_s:
        try:
            ...
            if not observation_keys_logged:
                logging.info("Observation image keys: %s", [
                    key for key in observation if key.startswith("observation.images.")
                ])
                observation_keys_logged = True
            ...
            if display_cameras and not is_headless():
                ...
                for key in image_keys:
                    preview = cv2.cvtColor(observation[key].numpy(), cv2.COLOR_RGB2BGR)
                    preview = cv2.resize(preview, (640, 480))
                    cv2.rectangle(preview, (0, 0), (640, 32), (0, 0, 0), -1)
                    cv2.putText(preview, key, (10, 23), cv2.FONT_HERSHEY_SIMPLEX,
                                0.6, (255, 255, 255), 1, cv2.LINE_AA)
                    previews.append(preview)
                if previews:
                    if not preview_initialized:
                        cv2.namedWindow("Recording cameras", cv2.WINDOW_NORMAL)
                        cv2.resizeWindow("Recording cameras", 640 * len(previews), 480)
                        cv2.moveWindow("Recording cameras", 20, 60)
                        preview_initialized = True
                    cv2.imshow("Recording cameras", cv2.hconcat(previews))
                cv2.waitKey(1)
